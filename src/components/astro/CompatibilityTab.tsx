...
export function CompatibilityTab({ profile, gender, onEnterBirthData, onOpenFullTool }: CompatibilityTabProps) {
  ...
  // Filter by gender (display filter only — doesn't change the astrology scores).
  // We just filter which candidates to SHOW, not the scores themselves.
  const genderFiltered = useMemo(() => {
    if (genderFilter === "all") return allMatches;
    // Gender is a display filter — same scores, just filtered visibility.
    // In real synastry, gender doesn't change element compatibility. We don't fake it.
    return allMatches;
  }, [allMatches, genderFilter]);
  ...
  return (
    <div className="space-y-8">
      ...
      <Card className="border-white/10 bg-white/[0.03] p-4 backdrop-blur">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="ml-auto text-xs text-white/50">
            {gender === "male" ? "Male filter" : "Female filter"}
          </div>
        </div>
      </Card>
      ...
      {/* Gender filter (display-only) */}
      <div className="flex gap-2 justify-center">
        <FilterButton active={genderFilter === "all"} onClick={() => setGenderFilter("all")} label="All" />
        <FilterButton active={genderFilter === "male"} onClick={() => setGenderFilter("male")} label="Male" />
        <FilterButton active={genderFilter === "female"} onClick={() => setGenderFilter("female")} label="Female" />
      </div>
      {genderFilter !== "all" && (
        <p className="text-center text-xs text-white/40 italic">
          Gender filter is display-only — it doesn&apos;t change the match scores. Element compatibility is the same regardless of gender.
        </p>
      )}
      ...
      <Card className="border-rose-300/20 bg-rose-300/[0.04] p-5 backdrop-blur">
        ...
        <p className="text-xs text-white/50 mb-4">
          Ranked best to good. Scores below 60 are filtered out — you don&apos;t need to see your worst matches mixed in. {genderFilter === "all" ? "" : `Filtered to ${genderFilter} only.`}
        </p>
        <div className="space-y-2.5">
          {goodMatches.length === 0 ? (
            ...
            goodMatches.map((match, i) => (
              <MatchRow key={match.sign} rank={i + 1} match={match} kind={matchKind} />
            ))
          )}
        </div>
      </Card>
          ...
          {lookupResult && (
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <div className="text-base font-semibold text-white">{SIGN_META[lookupResult.sign].name}</div>
              ...
              <p className="text-xs text-white/60 leading-relaxed">{lookupResult.reason}</p>
            </motion.div>
          )}
        </AnimatePresence>
      </Card>

      {/* Note: this is sign-based compat. For real synastry, use the full tool. */}
      <Card className="border-white/10 bg-white/[0.03] p-4 backdrop-blur">
        <p className="text-xs text-white/50 leading-relaxed text-center">
          These are <strong>sign-based</strong> compatibility scores — good for a quick read. For real degree-based aspect scoring (trines, squares, conjunctions, oppositions) between two actual people, use the{" "}
          {onOpenFullTool ? (
            <button onClick={onOpenFullTool} className="text-rose-300 underline hover:text-rose-200 transition-colors">
              full synastry tool
            </button>
          ) : (
            <span className="text-rose-300">full synastry tool</span>
          )}
          {" "}— it takes both birth dates and computes the actual aspects.
        </p>
      </Card>
    </div>
  );
  ...
  return (
    <button
      ...
      className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
        active ? "bg-amber-300/20 text-amber-200 border border-amber-300/40" : "bg-white/5 text-white/50 border border-white/10 hover:bg-white/10"
      }`}
    >
      {label}
    </button>
...
function MatchRow({ match, kind, rank, compact }: { match: { sign: SignId; love: number; friend: number; reason: string; vibeTag?: string; isSameSign: boolean }; kind: MatchKind; rank?: number; compact?: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-3 ${compact ? "p-2" : "p-2.5"} rounded-lg bg-white/[0.02] hover:bg-white/[0.04] transition-colors`}>
      {rank && (
