...
function getAspectExplanation(pair: PlanetPairAspect): string {
  ...
  if (aspectLower === "opposition") {
    const oppositionTexts: Record<string, string> = {
      ...
      "moon-saturn": `Your emotional needs and your sense of responsibility are at odds. The part of you that needs comfort and the part that says "buck up" are in constant negotiation. You'll go back and forth between "I need to feel this" and "I need to control this."`,
...
export function interpretNatalAspects(profile: NatalProfile): AspectInterpretation[] {
  ...
  const points: { id: string; name: string; absPos: number }[] = planets
    .filter((p) => typeof p.absPos === "number")
    .map((p) => ({ id: p.id, name: p.name, absPos: p.absPos as number }));
  if (angles) {
