...
// Everything below is only rendered AFTER the user submits a birth form or
// clicks a tab, so each view (and the astrology engine it uses) loads on
// demand. Same components, same content — they just stop blocking the first
// paint of the site. One shared spinner keeps the placeholder on-theme.
...
const lazyTab = <P,>(importer: () => Promise<{ default: React.ComponentType<P> }>) =>
  dynamic(importer, { ssr: false, loading: LoadingBlock });

const QuickView = lazyTab(() => import("@/components/astro/QuickView").then((m) => ({ default: m.QuickView })));
const DetailedView = lazyTab(() => import("@/components/astro/DetailedView").then((m) => ({ default: m.DetailedView })));
const CompatibilityChecker = lazyTab(() => import("@/components/astro/CompatibilityChecker").then((m) => ({ default: m.CompatibilityChecker })));
const CompatibilityTab = lazyTab(() => import("@/components/astro/CompatibilityTab").then((m) => ({ default: m.CompatibilityTab })));
const RedFlagsTab = lazyTab(() => import("@/components/astro/RedFlagsTab").then((m) => ({ default: m.RedFlagsTab })));
const KinkTestTab = lazyTab(() => import("@/components/astro/KinkTestTab").then((m) => ({ default: m.KinkTestTab })));
const SoulmateTab = lazyTab(() => import("@/components/astro/SoulmateTab").then((m) => ({ default: m.SoulmateTab })));
const AboutTab = lazyTab(() => import("@/components/astro/AboutTab").then((m) => ({ default: m.AboutTab })));
              ...
              {stage === "quick" && profile && (
                ...
                >
                  <QuickView
                    profile={profile}
                    onReadMore={() => {
                      document.getElementById("full-reading")?.scrollIntoView({ behavior: "smooth" });
                    }}
                    onReset={reset}
                  />

                  {/* Full chart + houses — shown inline on scroll, no click needed */}
                  <div id="full-reading" className="pt-8 border-t border-white/10">
                    <DetailedView profile={profile} />
                  </div>
                </motion.section>
              )}
