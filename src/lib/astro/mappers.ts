...
export function mapNatalProfile(resp: NatalApiResponse): NatalProfile {
  ...
  const ascAbsPos = angles?.asc ?? firstHouse?.abs_pos ?? sun.abs_pos;
...
// No jargon — just a quick, clear idea.
function plainAspectSummary(aPoint: string, bPoint: string, aspect: string, harmonious: boolean, tense: boolean): string {
  ...
  if (harmonious) {
    return `Your ${aRole} and their ${bRole} work together naturally. ${connection} This makes the relationship feel easy in this area.`;
  }
  if (tense) {
    return `Your ${aRole} and their ${bRole} create some friction. ${connection} This isn't a dealbreaker — it's just an area where you'll need to be more patient with each other.`;
  }
  // Neutral (conjunction)
  return `Your ${aRole} and their ${bRole} are in the same place. ${connection} This creates intensity — you amplify each other here, for better or worse.`;
}
...
function planetPairConnection(aPoint: string, bPoint: string): string {
  ...
  return map[pair] || `This connects your ${planetRoleShort(aPoint)} with their ${planetRoleShort(bPoint)}. When these two areas of life touch each other in a relationship, they create a specific channel through which you affect each other — sometimes harmoniously, sometimes with friction, but always meaningfully. The sign placement of each planet tells you HOW this plays out; the aspect between them (conjunction, trine, square, etc.) tells you whether it flows easily or requires work.`;
}
...
// Casual, deeper narrative explaining WHY the score is what it is.
// Walks through Sun/Moon/Rising comparison + the top aspects, in plain English.
function buildNarrative(opts: {
  overall: number;
  scoreBand: string;
  personA: { sun: SignId; moon: SignId; ascendant: SignId; venus?: SignId; mars?: SignId; mercury?: SignId; jupiter?: SignId; saturn?: SignId };
  personB: { sun: SignId; moon: SignId; ascendant: SignId; venus?: SignId; mars?: SignId; mercury?: SignId; jupiter?: SignId; saturn?: SignId };
  topStrengths: CompatibilityPairItem[];
  topFrictions: CompatibilityPairItem[];
  archetype: { label: string; one_liner: string };
}): string {
  const { overall, scoreBand, personA, personB, topStrengths, topFrictions, archetype } = opts;

  const bandWord =
    scoreBand === "exceptional" ? "really strong" :
    scoreBand === "strong" ? "solid" :
    scoreBand === "balanced" ? "right in the middle" :
    scoreBand === "moderate" ? "on the gentler side" :
    "pretty low-key";

  const parts: string[] = [];

  parts.push(`You two landed at ${overall} out of 100, which is ${bandWord}. That's not a verdict \u2014 it's a starting point. Here's what's actually going on underneath that number.`);

  // Sun comparison
  const sunSame = personA.sun === personB.sun;
  const sunCompat = elementCompat(personA.sun, personB.sun);
  if (sunSame) {
    parts.push(`Your Sun signs are the same \u2014 both ${signName(personA.sun)}. That's a real advantage: you fundamentally get each other's core identity. You don't have to explain yourself the way you do with most people. The downside is you might reinforce each other's blind spots, since you've got the same ones.`);
  } else {
    parts.push(`Your Sun signs are ${signName(personA.sun)} and ${signName(personB.sun)} \u2014 ${sunCompat}. This is the headline of how your core selves meet. ${sunImplication(personA.sun, personB.sun)}`);
  }

  // Moon comparison
  const moonSame = personA.moon === personB.moon;
  if (moonSame) {
    parts.push(`Your Moons are both in ${signName(personA.moon)} \u2014 your inner emotional worlds actually match. This is huge for living together or being close long-term. You'll soothe each other the same way, get upset about the same things, and recharge the same way.`);
  } else {
    const moonCompat = elementCompat(personA.moon, personB.moon);
    parts.push(`Your Moons are in ${signName(personA.moon)} and ${signName(personB.moon)} \u2014 ${moonCompat}. Moon compatibility is the secret sauce of feeling truly comfortable with someone. ${moonImplication(personA.moon, personB.moon)}`);
  }

  // Rising comparison
  const ascSame = personA.ascendant === personB.ascendant;
  if (ascSame) {
    parts.push(`You both have ${signName(personA.ascendant)} rising, so you come across to the world in a similar way. People probably meet you two and feel a similar energy from both of you.`);
  } else {
    parts.push(`Your rising signs are different (${signName(personA.ascendant)} and ${signName(personB.ascendant)}), so the first impression you each give is different. That's not bad \u2014 it just means people might be surprised when they meet you together.`);
  }

  // Top strengths
  if (topStrengths.length > 0) {
    const s = topStrengths[0];
    parts.push(`The strongest single connection between your charts is ${s.title.toLowerCase()} (${Math.round(s.strength * 100)}% strength). ${s.summary} That's the kind of contact that holds a relationship together when other things get bumpy.`);
    if (topStrengths.length > 1) {
      parts.push(`There are ${topStrengths.length - 1} more harmonious contacts supporting the bond, mostly around ${topStrengths.slice(1, 4).map(x => x.themes[0] || x.aspect).filter(Boolean).join(", ")}.`);
    }
  }
  ...
  if (topFrictions.length > 0) {
    ...
    parts.push(`On the other side, the biggest friction point is ${f.title.toLowerCase()} (${Math.round(f.strength * 100)}% strength). ${f.summary} This isn't a dealbreaker \u2014 it's just the place where you'll have to actually do the work of understanding each other instead of expecting it to be easy.`);
    if (topFrictions.length > 1) {
      parts.push(`There are ${topFrictions.length - 1} more tension contacts to be aware of, mostly around ${topFrictions.slice(1, 4).map(x => x.themes[0] || x.aspect).filter(Boolean).join(", ")}.`);
    }
  } else {
    parts.push(`There are no major friction contacts in your charts \u2014 which is unusual and good, but also means the relationship might lack the spark that comes from a little healthy friction. Don't manufacture drama where there isn't any.`);
  }

  // Archetype framing
  parts.push(`The pattern across your whole chart is "${archetype.label.toLowerCase()}." ${archetype.one_liner}`);

  // Closing
  parts.push(`So: ${overall} isn't a grade, it's a map. The high domains are where you can lean in and trust the connection. The low ones are where you'll need to talk more, slow down, and not assume the other person sees it your way.`);
...
function buildTensionPoints(frictions: CompatibilityPairItem[]): TensionPoint[] {
  ...
  // If we have fewer than 3 from aspects, add a few general ones based on
  // the planets involved (Sun/Moon/Venus/Mars are the everyday friction zones).
  return out;
}
...
export function mapSynastryProfile(resp: SynastryApiResponse): CompatibilityProfile {
  ...
  const overall = syn.scores.overall;
  const scoreBand =
  ...
  const tensionPoints = buildTensionPoints(topFrictions);
  ...
  return {
    ...
    overall,
    domainScores: [
      { key: "romance", label: "Romance", value: syn.scores.romance },
      { key: "communication", label: "Communication", value: syn.scores.communication },
      { key: "stability", label: "Stability", value: syn.scores.stability },
      { key: "intimacy", label: "Intimacy", value: syn.scores.intimacy },
      { key: "growth", label: "Growth", value: syn.scores.growth },
    ],
    archetype: syn.archetype,
    narrative,
