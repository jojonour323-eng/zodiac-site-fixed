...
function moonInSign(s: SignId): { short: string; positive: string; shadow: string; takeaway: string } {
  const map: Record<SignId, { short: string; positive: string; shadow: string; takeaway: string }> = {
    ...
    gemini: {
      ...
      shadow: "You can talk about feelings instead of actually feeling them. You'll talk around something for hours without ever landing on it. Restlessness means you flee emotionally heavy moments instead of staying.",
  ...
  const traits: Record<PlanetId, Partial<Record<SignId, TraitPoint[]>>> = {
    ...
    moon: {
      ...
      gemini: [
        ...
        { label: "YOU STRUGGLE", text: "to actually feel things — you talk about them instead" },
...
Real moment: say something goes wrong in ${houseName}. Your Moon doesn't just think 'that's annoying' \u2014 it feels it. ${moonReactionLine(moonSign)} That's not overreacting. That's your Moon doing its job, in the house where it lives.`;
}
...
Real moment: say you're into someone. Your ${venusMeta.name} Venus is doing the wanting \u2014 you notice them, you feel the pull, you imagine what it would be like. Your ${marsMeta.name} Mars is doing the chasing \u2014 the actual texting, the asking out, the making it happen. If those two energies are out of sync (say, Venus wants deep connection but Mars wants the thrill of the chase), you'll feel torn in two directions inside the same situation. The trick is naming both: 'I'm drawn to this person because [Venus reason], and I'm approaching it this way because [Mars reason].' Once you can see both, you can choose.
...
const PLANET_HOUSE_REDIRECT: Record<string, (houseName: string) => string> = {
  sun: (h) => `In the ${h}, your core identity gets redirected. But the real shift is: instead of your identity being about who you are in general, it's specifically about who you are in the realm of ${h}.`,
...
export function detectChartPatterns(profile: NatalProfile): { title: string; teaser: string; body: string }[] {
  ...
  if (sunEl !== moonEl) {
    const tensionMap: Record<string, string> = {
      "fire-earth": "Your identity wants action and risk; your emotional world wants stability and comfort. You'll go back and forth between 'let's go' and 'let's stay.'",
      "fire-water": "Your identity wants to express and act; your emotional world wants to feel and process. You'll go back and forth between doing and being.",
      "fire-air": "Your identity wants passion and directness; your emotional world wants understanding and distance. You'll go back and forth between heart and head.",
      "earth-air": "Your identity wants tangible results; your emotional world wants ideas and connection. You'll go back and forth between building and thinking.",
      "earth-water": "Your identity wants structure and practicality; your emotional world wants depth and feeling. You'll go back and forth between holding and flowing.",
      "air-water": "Your identity wants to understand and communicate; your emotional world wants to feel and merge. You'll go back and forth between thinking and feeling.",
    };
